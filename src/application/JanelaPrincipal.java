...
import entities.CasaGUI;
import entities.Jogo;
import entities.TabuleiroGUI;
import entities.pecas.Rei;
...
public class JanelaPrincipal extends JFrame {
    ...
    public void reagir(CasaGUI casaClicada) {
        if (primeiroClique) {
            ...
            if(jogo.isPecaPromovida()) {
                boolean escolhaValida = false;
                while(!escolhaValida) {

                    String[] pecasDisponiveis = {"RAINHA", "TORRE", "BISPO", "CAVALO" };
                    String pecaEscolhida = (String) JOptionPane.showInputDialog(null, "Escolha um item", "Op�ao",
                            JOptionPane.INFORMATION_MESSAGE, null, pecasDisponiveis, pecasDisponiveis [0]); //

                    //String pecaEscolhida = JOptionPane.showInputDialog("Escolha a peca que ser� promovida: (RAINHA, TORRE, BISPO, CAVALO");

                    escolhaValida = jogo.promocaoPeca(jogo.getTabuleiro().getCasa(casaClicadaDestino.getPosicaoX(),
                            casaClicadaDestino.getPosicaoY()).getPeca(), pecaEscolhida);
                }
            }
    ...
    /**
     * Se um dos reis est� em xeque envia mensagem informando
     */
    private void xequeMensagem() {

        Rei reiBranco = (Rei) jogo.getCasaReiBranco().getPeca();
        Rei reiPreto = (Rei) jogo.getCasaReiPreto().getPeca();
        jogo.setReiPretoXeque(reiPreto.xeque(jogo.getCasaReiPreto()));
        jogo.setReiBrancoXeque(reiBranco.xeque(jogo.getCasaReiBranco()));

        if(jogo.isReiBrancoXeque() || jogo.isReiPretoXeque()) {

            String corRei;
            if(jogo.isReiBrancoXeque()) {
                corRei = "Branco";
            }
            else {
                corRei = "Preto";
            }
            JOptionPane.showMessageDialog(this, "Rei " + corRei + " em Xeque");
        }
    }

    /**
     * Se o jogo acabou, envia mensagem em caso de algum clique no tabuleiro
     */
    private void gameOverMensagem() {
        if(jogo.isGameOver()) {

            String vencedor;
            if(jogo.getTurnoCor() == PecaCor.BRANCA) {
                vencedor = "Preto";
            }
            else {
                vencedor = "Branco";
            }
            JOptionPane.showMessageDialog(this, "Jogo vencido pelo Jogador " + vencedor);
        }
    }
